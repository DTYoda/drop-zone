...
Once both sides have a verified reply, they keep punching for a short grace
period so the slower peer still receives PunchAcks, then run the reliable UDP
protocol below. Hearing a probe without a matching PunchAck is not enough: both
peers must confirm the path, otherwise one side can commit to UDP while the
other falls through to the relay.
